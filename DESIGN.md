...
Layout (revised for the Guessmoji redesign, superseding the single-column version below): a wide 3-column shell — leaderboard (270px) | emoji stage (flexible) | chat/guess panel (300px) — on desktop (`≥lg`). Chat and guessing are no longer visually separate cards — one panel owns both, with the input as its footer.

**Mobile (`<lg`), revised for the skribbl-style relayout — supersedes the earlier "stage, then leaderboard, then chat" single-column stack**: a bounded, non-scrolling 3-row shell instead. Row 1 is the clue box (emoji sequence + letter-hint blanks), full viewport width, capped at `36dvh` rather than filling the screen. Row 2 splits the remaining height evenly between a compact leaderboard (left) and chat (right), each scrolling internally if its content overflows. Row 3 is the guess/chat input as its own full-viewport-width bar below rows 1–2, edge to edge like skribbl's input — not confined to the chat column's width. The page itself never scrolls (`100dvh`-bounded shell); only rows 2's two panels do. The mobile leaderboard is a genuinely compact row (avatar + short name + score) — no substatus line, no rank medal, no "next up" box; the current player's own row gets a ring highlight in place of the desktop "you" pill, which doesn't fit at this width.

- **Emoji sequence** appears centered, large, as soon as the round starts, under a "decode this" label (desktop only — dropped on mobile to save vertical room in the capped stage) — no "Round N of M" on the card itself anymore, that moved to the page header alongside the room's name and category.
- **Letter-hint blanks**: below the emoji sequence, underscores for the answer (word-boundary spacing and punctuation preserved, e.g. `can't stop` → `_ _ _ ' _   _ _ _ _`), fully blank for the round's first 15 seconds, then a handful of non-first letters reveal in phases up to 85% of the round's duration. Reveal count is tiered by answer shape: a single word under 4 letters gets at most 1 reveal, a single word of 4+ letters gets at most 2, and any multi-word answer gets at most 3 (each further capped so at least one eligible letter always stays blank). A word's first letter is never revealed — too strong a single hint — and which non-first letters get picked is deterministic per round (seeded by the round id), so a mid-round refresh shows the same letters rather than re-rolling them. Server-computed on every `round-tick` response from the real answer and the request's own clock — never persisted or broadcast, so an unrevealed letter is never present in anything the client can read ahead of time (CLAUDE.md rule 1; see `supabase/functions/_shared/hint.ts`).
...
- **Not yet built**: letter-blank hints shipped (see above); a live "Sam is very close 🔥" near-miss line is still out of scope (needs edit-distance scoring). Lobby chat is the same story — `submit-guess` is round-scoped today, so pre-game chat doesn't exist yet.
...
**Layout:** centered single column (max content width ~640–720px) for the home/create/join/lobby screens; the round loop and results screens are a wide 3-column shell up to 1280px (§2.4) on desktop. Below `lg`, the results screen still stacks to a single scrolling column, but the round loop instead uses a bounded, non-scrolling skribbl-style 3-row shell — see §2.4 for the full mobile spec, it supersedes the single-column description for that one screen. 24–32px padding, minimal chrome (small home link only).
